...
from app.services import patinent_service
...
class PatientList(Resource):
    def get(self):
        users = patinent_service.get_all_users()
        return jsonify(users)

    def post(self):
        data = request.get_json()
        new_user = patinent_service.create_user(data)
        return jsonify(new_user), 201

@patient_ns.route("/<int:user_id>")
class PatientDetail(Resource):
    def get(self, user_id):
        user = patinent_service.get_user_by_id(user_id)
        if user:
            return jsonify(user)
    ...
    def put(self, user_id):
        data = request.get_json()
        updated_user = patinent_service.update_user(user_id, data)
        if updated_user:
...
class PatientDisable(Resource):
    def put(self, user_id):
        disabled_user = patinent_service.disable_user(user_id)
        if disabled_user:
